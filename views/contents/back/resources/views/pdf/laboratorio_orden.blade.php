...
<body style="font-family: 'Times New Roman', Times, serif;">
  ...
  <br>
    <div class="header" style="margin-top: -35px;">
        <h2 style="font-family: 'Times New Roman', Times, serif;">ORDEN CLINICA - EXAMENES DE LABORATORIO</h2>
    </div>
    <hr>
    <hr>
    <div class="col-12">
    ...
    <div class="col-12 ">
        ...
        <div class="col-6">
            ...
            <br>
            <ul>
                @if(isset($response['citas']['laboratorio']) && count($response['citas']['laboratorio']) > 0)
                @foreach($response['citas']['laboratorio'] as $lab)
                @foreach($lab['laboratorio_detalle'] as $detalle_lab)
                @if($detalle_lab['estado'] == 'A')
                <li>
                    {{ $detalle_lab['tipo_examen']['codigo_lab'] }} -
                    {{ $detalle_lab['tipo_examen']['descripcion_lab'] }} -
                    {{ $detalle_lab['tipo_examen']['categoria_laboratorio']['descripcion_categoria'] }}
                </li>
                @endif
                @endforeach
                @endforeach
    ...
    <div class="firma" style="line-height: 0.9; font-family: 'Times New Roman', Times, serif;">
        ...
        <p>Dr. {{ ($response['citas']['doctor']['persona']['nombre'] ) }}
            {{ ($response['citas']['doctor']['persona']['apellido'] ) }}
        </p>
        ...
        <p>Registro Senescyt: {{ ($response['citas']['doctor']['reg_senescyt'] ) }} - Cédula:
            {{ ($response['citas']['doctor']['persona']['cedula'] ) }}
        </p>
        <p>Registro MSP:</p>
        <p></p>
