...
/**ELIMINAR MODAL
 * 
 * ESTE SI FUNCIONA ESTOS 3 METODOS PERO CON EL MODAL 
 */

/* 
function eliminar_detalle(id) {
  alert(id);
  $('#modalTratamientoEliminar').modal('show');
  cargarOdoIdDatosEliminar(id);
}
function cargarOdoIdDatosEliminar(id) {
  $.ajax({
    url: urlServidor + 'odontograma/listarodontogramadetallexid/' + id,
    type: 'GET',
    headers: { "Authorization": `Bearer ${token}` }, 
    ...
    success: function (response) {
      console.log(response);
      if (response.status) {
        $('#eliminar-detodonto-id').val(id);
        $('#eli-numero-pieza').val(response.ododetalle.pieza);
        $('#eli-cuadrante').val(response.ododetalle.cuadrante);
  
      // let estadoTratamiento = response.ododetalle.estado; // Recuperar el estado desde la BD
        // Establecer el valor en el select de estado
    //    $('#EestadoTratamientoEliminar').val(estadoTratamiento); 

        $('#EestadoTratamientoEliminar').val("Vacio"); // Preseleccionado por código

        let estadoTratamiento = $('#EestadoTratamientoEliminar').val();
           ...
           if (estadoTratamiento === "Por hacerse") {
               ...
               caraElemento.addClass("estado-realizado");
           }else if (estadoTratamiento === "Vacioo") {
 ...
 function eliminandodetalleOdonto() {

  $('#EguardarEliminar').click(function () {
  
    

      let id = $('#eliminar-detodonto-id').val();
      let pieza = $('#eli-numero-pieza').val();
      let cuadrante= $('#eli-cuadrante').val();
      let estado = $('#EestadoTratamientoEliminar option:selected').val();
        ...
        $.ajax({
          ...
          success: function (response) {
              ...
              if (response.status) {
                ...
                $('#modalTratamientoEliminar').modal('hide'); 
                   // Actualizar el color en el odontograma al cambiar el estado
                   let caraElemento = $('.diente[data-numero="' + pieza + '"] .cara[data-cara="' + cuadrante + '"]');
                   caraElemento.removeClass("estado-por-hacer estado-encontrado estado-realizado estado-vacio");
