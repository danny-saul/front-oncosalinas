$(function () {
    ...
    function guardarUsuario() {
        $('#nuevo-usuario').submit(function (e) {
            ...
            let roles_id = $('#nuevo-rol option:selected').val();
            ...
            let img_sello = $('#img-sello')[0].files[0];
            let def2 = (img_sello == undefined) ? 'sello.png' : img_sello.name; 
    
            let especialidades_id = $('#nuevo-especialidad option:selected').val();
            let json = {
                usuario: {
                    roles_id,
                    usuario,
    ...
    function guardandousuario(json) {
       peticionJWT({
        ...
        if (json.doctor.img_sello == 'sello.png') {
...
/*     function cargarSexo() {
    ...
    function cargarEspecialidad() {
       peticionJWT({
            ...
            success: function (response) {
                console.log(response);
                if (response.status) {
                    let data = response.especialidades.map(element => ({
                        id: element.id,
                        text: element.nombre_especialidad
                    }));
    
                    $('#nuevo-especialidad').select2({
                        data: data,
    ...
    function cargarInputsOcultos() {
        $('#nuevo-rol').change(function () {
            let rol_id = $('#nuevo-rol option:selected').val();
    
            if (rol_id == '2') {
                $('#dt-especialidad').removeClass('d-none');
                //$('#datos-representante').addClass('d-none');
            } else
                if (rol_id == '1') {
                    $('#dt-especialidad').addClass('d-none');
                    //$('#datos-representante').addClass('d-none');
                } else
                    if (rol_id == '3') {
                        $('#dt-especialidad').addClass('d-none');
